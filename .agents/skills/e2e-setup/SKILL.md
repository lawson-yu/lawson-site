...
E2e tests verify the whole running system _through the app_ (browser/API), not one
...
- **Verify auth ITSELF once; bypass it everywhere else.** A dedicated signup/login
  spec proves auth works. Every _other_ spec shouldn't re-pay the login tax — build
...
- **Layered assertions: client → server → product.** Don't stop at "the UI changed."
  ...
  user-visible outcome (e.g. plan upgraded _and_ credits granted).
...
when the _intended contract_ changed — confirmed from the diff, not assumed.
