...
1. **Artifacts** are global, foldered by **kind**; `domain:` is a **field (a list)**, not a folder.
   Each artifact has exactly one home (by _what it is_). Cross-cutting is handled by tags + links
...
dated: `YYYY-MM-DD | source — what happened`). _"What's true now"_ = body; _"what happened"_ =
...
- **`domains/<x>/metrics/*.jsonl`** — numeric time-series, written by **deterministic collectors**
  (code/skills, _not_ the LLM). Agents read & interpret. Scorecards are generated from these.
...
3. **Half-nested** (some kinds global, some under domains). ❌ The asymmetry _is_ the bug.
4. **Pure database** (Notion-style). ❌ for now — we want the data to live in _this_ repo
...
properties + relations + _views_; nesting is data, not folders); **markdown-as-system-of-record**
