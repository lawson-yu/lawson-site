...
test("访客可搜索已发布内容、按类型筛选，且搜索页不被索引", async ({ page }) => {
  ...
  await page.goto("/zh-CN/search?q=LAWSON&kind=project");
  await page.getByRole("link", { name: "LAWSON Site", exact: true }).click();
  await expect(page).toHaveURL(/\/zh-CN\/projects\/lawson-site$/);

  await page.goto("/zh-CN/search?q=LangChain&kind=curated");
  const resultTag = page.locator("article").first().getByRole("link").nth(1);
  await expect(resultTag).toBeVisible();
  await resultTag.click();
  await expect(page).toHaveURL(/\/zh-CN\/curated\?tag=/);
